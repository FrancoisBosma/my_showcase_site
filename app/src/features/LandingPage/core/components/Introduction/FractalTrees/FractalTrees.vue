...
<style scoped lang="postcss">
  .fractals-tree {
    @apply absolute bottom-16 right-0 mr-16 w-72 h-36 flex flex-col justify-center items-center;
    .bg-fractals-toggle-buttons {
      ...
      .tree-icon {
        @apply absolute w-18 h-18 cursor-pointer transition-all duration-300 ease-in-out;
      }
      .icon {
        @apply relative w-[calc(18rem*1.13/4)] h-[calc(18rem*1.13/4)] -top-1 -left-1 cursor-pointer
      ...
      &[is-active='false'] {
        ...
        .tree-icon {
          @apply transform translate-x-114/100 rotate-90 opacity-25;
        }
        .add {
          @apply opacity-100;
        }
